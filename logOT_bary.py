...
def myAssymetricWellCost(Nr, Nc, divmed=False, timeit=False, trunc=False, maxtol=745.13, truncval=745.13):
    if timeit:
        start = time.time()
    N = Nr * Nc
    C = np.zeros((N,N))
    ...
    for k1 in range(N):
        for k2 in range(k1):
            r1, r2 = int(float(k1) / Nc)+1, int(float(k2) / Nc)+1
            c1, c2 = k1%Nc + 1, k2%Nc + 1
            r1_,r2_ = abs(int(float(k1) / Nc) -21 -np.sqrt(2.0)/2.0),abs(int(float(k2) / Nc)-21- np.sqrt(2.0)/2.0)
            c1_, c2_ = abs(k1%Nc -21)+1, abs(k2%Nc -21)+1
          
            
            rho_2, theta_2 = utils.cart2pol(r2_,c2_)
            rho_1, theta_1 = utils.cart2pol(r1_,c1_)
#             C[k1, k2] = (rho_2/rho_1)  
            C[k1, k2] = (r1-r2)**2 + (c1-c2)**2
            
        
#             import pdb; pdb.set_trace()  # breakpoint bc817e81 //
            lower.append(C[k1, k2])
            if k1 <> k2:
...
#                 import pdb; pdb.set_trace()  # breakpoint bc817e81 //
                ...
                C[k2, k1] =  C[k1, k2]
            upper.append(C[k2, k1])
            
    cte = np.max(lower)/np.max(upper)
    print "max lower", np.max(lower)
    ...
    for k1 in range(N):
        for k2 in range(k1):
            C[k2, k1] *=  cte
            upper_new.append(C[k2, k1])
#     print "max new upper ", np.max(upper_new)
...
Dlog_stack = T.tensor3('Dlog_stack')
...
def varchange(i,newvar):
    return T.exp(newvar[:,i,:])/T.sum(T.exp(newvar[:,i,:]),axis=0)

D_stack, D_stack_updates = theano.scan(varchange,sequences=T.arange(Dlog_stack.shape[1]),non_sequences=[Dlog_stack])

D_stack = T.swapaxes(D_stack,0,1)

Theano_varchange = theano.function([Dlog_stack], D_stack)
...
Theano_bary = theano.function([D_stack,lbda_stack,Gamma,Cost,n_iter,theano.In(Tau,value=-0.3),theano.In(Epsilon,value=1e-200)],log_result_bary_rshp,on_unused_input='ignore')
...
res_B_,updates_B = theano.scan(lin_comb_B,#for each object
                             outputs_info = None,
...
Theano_wdl_MtX = theano.function([A_mat,beta_mat,Flux_all,Sigma_all,K_all,D_stack,lbda_stack,Cost,Gamma,n_iter,Datapoint,\
	theano.In(Tau,value=-0.0),theano.In(Epsilon,value=1e-200)],[MtX_wdl,res_B_rshp,barycenters],on_unused_input='ignore',mode="FAST_COMPILE")

MtX_wdlLog = T.grad(Loss, Dlog_stack)

Theano_wdlLog_MtX = theano.function([A_mat,beta_mat,Flux_all,Sigma_all,K_all,Dlog_stack,lbda_stack,Cost,Gamma,n_iter,Datapoint,\
    theano.In(Tau,value=-0.0),theano.In(Epsilon,value=1e-200)],[MtX_wdlLog,res_B_rshp,barycenters],on_unused_input='ignore',mode="FAST_COMPILE")

Theano_wdl_MX = theano.function([A_mat,beta_mat,Flux_all,Sigma_all,K_all,D_stack,lbda_stack,Cost,Gamma,n_iter,\
	theano.In(Tau,value=-0.0),theano.In(Epsilon,value=1e-200)],[res_B_rshp,barycenters,log_lKtas_rshp,log_betas_rshp],on_unused_input='ignore')

MtX_coeff = T.grad(Loss,A_mat)

Theano_coeff_MtX = theano.function([A_mat,beta_mat,barycenters,Flux_all,Sigma_all,K_all,Datapoint],[MtX_coeff,res_B_rshp],on_unused_input='ignore')
