...
for obj,pos in enumerate(np.random.randint(output_stack.shape[2],size=nb_obj)):
...
lbdas = np.arange(550.0,970.0,70.0)#min step = 5, remember to put 1 step forward in right limit of arange
nb_wvl = lbdas.size
SEDs = SEDs[::14,:]
gt = gt[:,:,::14,:]
...
psf_est,D,A,res,obs_est =  polychromatic_psf_field_est_2(stars,SEDs,lbdas,D,opt_shift,nb_comp,gt,nb_iter=1,nb_subiter=3,mu=0.3,\
...
save_path = '/Users/rararipe/Documents/Data/lbdaRCA_wdl/euclid/42x42pixels_6lbdas10pos/'                       
