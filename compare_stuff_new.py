...
all_lbdas = np.load(data_path_8wvl + 'all_lbdas.npy')
...
for it in range(nb_altiter):
    fig = plt.figure()
    plt.errorbar(all_lbdas,np.mean(abs(MSE_rel_nor_alternates_8wvl[it,:,:]),axis= 0),yerr=np.std(abs(MSE_rel_nor_alternates_8wvl[it,:,:]),axis= 0)  ,label='8 bins', color='darkorchid',capsize=3, alpha=.7)
    plt.errorbar(all_lbdas,np.mean(abs(MSE_rel_nor_alternates_10wvl[it,:,:]), axis= 0),yerr = np.std(abs(MSE_rel_nor_alternates_10wvl[it,:,:]), axis= 0), label='10 bins', color='red',capsize=3, alpha=.7)
